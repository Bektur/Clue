...
% this is to clear the dynamic variable every time we consult 
% the game
:- abolish(setofSolutions/1).
...
% generate all the available solutions of three cards and save them in the 
% dynamic variable setofSolutions 
generateSol(L):-
	weapon(X),
	L = [X|T],
	generateChar(T),
	assert(setofSolutions(L)),
	generateSol(T).
...
% intersection predicate to decide the intersection of two lists 
intersection([],_,[]).
intersection([H1|T1],L2,[H1|T2]):-
	member(H1,L2),
